...
class MagHeading():
    def __init__(self):
        ...
        self.magHdg_pub = rospy.Publisher('compass', Float32, queue_size=1)
        self.world_imu_pub = rospy.Publisher ('global_imu', Imu, queue_size=1)
        #self.br = tf.TransformBroadcaster()

        # Subscribe to the gps positions
        rospy.Subscriber('magnetic', Vector3Stamped, self.update_magnetic_callback)
        rospy.Subscriber('imu_raw', Imu, self.update_imu_callback)
